...
export const SuperAdmin = () => {
  ...
  // Handlers for Forms... (omitted to keep concise as they exist)
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-gray-900/50 text-gray-400 text-xs uppercase font-bold">
                        <tr>
                            ...
                            <th className="p-4 text-right">Inventory Sales Value</th>
                                ...
                                <tr key={b.id} className="hover:bg-gray-700/50 transition cursor-pointer group" onClick={() => setSelectedBranchForDetails(b)}>
                                    ...
                                    <td className="p-4 text-center" onClick={e => e.stopPropagation()}>
                                        <div className="flex justify-center gap-2">
                                            <button onClick={() => handleGenerateBranchFinancials(b, 'pdf')} className="bg-red-900/50 text-red-400 p-2 rounded hover:bg-red-900 transition" title="Download PDF"><Icons.FileText size={16}/></button>
                                            <button onClick={() => handleGenerateBranchFinancials(b, 'print')} className="bg-blue-900/50 text-blue-400 p-2 rounded hover:bg-blue-900 transition" title="Print"><Icons.Printer size={16}/></button>
                                        </div>
                                    </td>
              ...
              <div className="max-w-xl mx-auto bg-gray-800 rounded-xl border border-gray-700 p-8">
                  ...
                  {isEditingProfile ? (
                      <form onSubmit={handleUpdateProfile} className="space-y-4">
                          <div><label className="block text-sm font-bold text-gray-400 mb-1">Full Name</label><input name="name" defaultValue={user?.name} className="w-full p-2 bg-gray-900 border border-gray-600 rounded text-white" required /></div>
                          <div><label className="block text-sm font-bold text-gray-400 mb-1">Username</label><input name="username" defaultValue={user?.username} className="w-full p-2 bg-gray-900 border border-gray-600 rounded text-white" required /></div>
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="flex justify-between p-3 bg-gray-900 rounded border border-gray-700"><span className="text-gray-400 font-medium">Account Status</span><span className="font-bold text-green-500 flex items-center gap-1"><Icons.Check size={14}/> Active</span></div>
                      </div>
                  )}
              </div>
          )}
      ...
      {selectedBranchForDetails && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
              <div className="bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col border border-gray-600">
                  <div className="p-6 border-b border-gray-700 flex justify-between items-center">
                      <div><h3 className="text-xl font-bold text-white">{selectedBranchForDetails.name} Inventory</h3><p className="text-gray-400 text-sm">Detailed Product List</p></div>
                          ...
                          <tbody className="divide-y divide-gray-700">
                              {branchDetailsProducts.map(p => (
                                  <tr key={p.id} className="hover:bg-gray-700/50">
                                      ...
                                      <td className="p-3 text-center"><span className={`px-2 py-0.5 rounded-full text-xs font-bold ${p.stock < p.minStockAlert ? 'bg-red-900/50 text-red-400' : 'bg-green-900/50 text-green-400'}`}>{p.stock}</span></td>
                  ...
                  <div className="p-6 border-t border-gray-700 bg-gray-800 rounded-b-xl flex justify-end"><button onClick={() => setSelectedBranchForDetails(null)} className="px-6 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded font-bold">Close Details</button></div>
              </div>
          </div>
      )}
